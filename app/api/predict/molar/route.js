...
export async function POST(request) {
  try {
    ...
    const essentialFields = ["age", "ageGroup"]; // At minimum, we should have age and ageGroup

    for (const field of essentialFields) {
      if (!formData[field] && formData[field] !== 0) {
        return NextResponse.json(
          {
            error: `Essential field missing: ${field}. At minimum, patient age and age group are required.`,
          },
          { status: 400 }
        );
      }
    ...
    // Optional: Clean up empty strings to undefined for better processing
